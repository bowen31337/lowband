...
> `claw-forge validate-spec` (strict shape is the default since v0.8.46).
...
> Strict shape is the default (since v0.8.46): shape gaps 3, 5, 9 and the
> migration-shape gap 8 are ERRORs (pass `--soft-shape` to opt out). Since
> `/import-spec` emits shape-annotated `<feature>` elements from every
> category, the imported spec should pass on the
